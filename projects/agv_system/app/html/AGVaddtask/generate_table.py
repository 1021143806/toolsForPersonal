#!/usr/bin/env python3
"""
从 index.html 中提取 config 对象并生成 markdown 表格
"""
...
def extract_config(html_content):
    ...
    # 移除注释行（可选）
    config_str = re.sub(r'//.*?\n', '\n', config_str)
    # 使用 json5 解析 JavaScript 对象
    try:
        config = json5.loads(config_str)
    except Exception as e:
        print(f"解析错误: {e}")
        # 尝试修复常见的 JavaScript 语法问题
        # 将单引号替换为双引号（json5 支持单引号，但为了安全）
        # 但 json5 应该能处理
        raise
...
def insert_table_into_readme(readme_path, table):
    """将表格插入到 readme.md 的合适位置"""
    with open(readme_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # 找到“系统配置”部分之后的位置，在“已配置区域”列表之后插入表格
    # 我们可以在“已配置区域”列表之后插入，或者新建一个“任务模板详情”章节
    # 根据现有内容，在“已配置区域”列表之后插入比较合适
    # 查找“## 已配置区域”部分
    pattern = r'(## 已配置区域.*?\n)(.*?)(?=\n##|\n---|\n\*|\n$)'
...
def main():
    ...
    # 插入到 readme.md
    insert_table_into_readme(readme_path, table)
    print(f"\n表格已插入到 {readme_path}")

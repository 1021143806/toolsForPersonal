...
function connectMsqlAgvWms($ip) //连接AGV数据库
{
  ...
  //print "测试用".$dbhost.$ip.$dbuser.$dbpass;
...
function connectMsqlAgvWmsNoINFO($ip) //连接AGV数据库
{
  global $dbhost, $dbuser, $dbpass, $conn, $ip1;
  // 要执行的代码
  $ip1 = $ip;
...
//连接其他数据库
function connectMySQLAll($ip,$dbuser,$dbpass,$MySQL) //连接其他数据库
{
  global $dbhost, $dbuser, $dbpass, $conn, $ip1;
  // 要执行的代码
  $ip1 = $ip;
...
function showdownMysqlNoINFO() //关闭数据库
{
  global $dbhost, $dbuser, $dbpass, $conn, $ip1;
  //释放内存
  mysqli_close($conn);
